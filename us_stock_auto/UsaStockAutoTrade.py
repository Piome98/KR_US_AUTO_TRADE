...
# 매수 목표가 -> 일봉 이동 평균선 기준 매수 목표가 산정 로직으로 수정 필요요
...
# 매수 주문 API 호출 -> 시장 이름, 종목 티커(코드), 종목 수량 -> 전체 시드의 25% 가능 수량, 가격-> 이동 평균선 기준(60, 120) 로직 수정 필요
...
# 매도 주문 API 호출 -> 마찬가지로 호출하되, 종목 코드는 잔고 조회 해서 계좌 내에 있는 종목에 대해서만 매도 주문 실행
...
def get_exchange_rate():
    ...
    res = requests.get(URL, headers=headers, params=params)
    # 그 날 환율 정보 크롤링 해서 exchange_rate에 넣어주는 로직 추가 필요요
