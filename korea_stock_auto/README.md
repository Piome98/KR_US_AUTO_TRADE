...
- **api_client.py**: 한국투자증권 API 클라이언트 클래스
- **auth.py**: API 인증 및 토큰 관리
- **websocket.py**: 실시간 시세 수신을 위한 웹소켓 클라이언트
...
### 2. 강화학습 모델 (rl_models/rl_model.py)

강화학습 모델 모듈은 트레이딩 환경 정의와 모델 구현을 담당합니다.

**주요 클래스:**

- **TradingEnvironment**: OpenAI Gym 기반의 트레이딩 환경
- **RLModel**: 강화학습 모델 인터페이스
- **ModelEnsemble**: 여러 모델을 앙상블하는 클래스
...
### 3. 모델 학습 및 평가 (training/trainer.py)
