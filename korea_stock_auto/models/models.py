...
class ModelManager:
    ...
    def load_latest_model(self):
        ...
        try:
            ...
            if not model_files:
                send_message("사용 가능한 모델 파일이 없습니다. 기본 전략을 사용합니다.", self.config.notification.discord_webhook_url)
            ...
            send_message(f"모델 로드 성공: {latest_model}", self.config.notification.discord_webhook_url)
            self.fallback_mode = False
            return True
            
        except Exception as e:
            send_message(f"모델 로드 실패: {e}", self.config.notification.discord_webhook_url)
            self.fallback_mode = True
            return False
    ...
    def predict(self, state):
        ...
        try:
            ...
            send_message(f"예측 실패: {e}", self.config.notification.discord_webhook_url)
            return self._fallback_strategy(state)
    ...
    def log_prediction(self, code, state, action, confidence, price):
        ...
        try:
            ...
            send_message(f"예측 로깅 실패: {e}", self.config.notification.discord_webhook_url) 