...
from korea_stock_auto.utils import send_message, hashkey
from korea_stock_auto.auth import get_access_token
...
class APIClient:
    ...
    def get_top_traded_stocks(self):
        ...
        url = f"{URL_BASE}/{path}"
        headers = self._get_headers("FHPST01710000" if USE_REALTIME_API else "FHPST01710000")
        params = {
    ...
    def get_stock_info(self, code):
        ...
        url = f"{URL_BASE}/{path}"
        headers = self._get_headers("FHKST01010100" if USE_REALTIME_API else "FHKST01010100")
        params = {
            "FID_COND_MRKT_DIV_CODE": "J",
            "FID_INPUT_ISCD": code
        }
        ...
        try:
            ...
            send_message(f"[오류] 종목 정보 조회 실패: {e}")
            return {}
    ...
    def get_stock_data(self, code, period_div_code):
        ...
        url = f"{URL_BASE}/{path}"
        headers = self._get_headers("FHKST01010400" if USE_REALTIME_API else "FHKST01010400")
        params = {
            "FID_COND_MRKT_DIV_CODE": "J",
            "FID_INPUT_ISCD": code,
        ...
        try:
            ...
            send_message(f"[오류] 차트 데이터 조회 실패: {e}")
            return []
