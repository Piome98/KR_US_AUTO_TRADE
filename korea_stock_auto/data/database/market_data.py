...
class MarketDataManager(DatabaseManager):
    ...
    def save_market_index(self, date: str, index_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            send_message(f"시장 지수 데이터 저장 실패: {e}", self.config.notification.discord_webhook_url)
            return False
    ...
    def get_market_index_history(self, index_code: str, days: int = 30) -> pd.DataFrame:
        ...
        try:
            ...
            send_message(f"시장 지수 이력 조회 실패: {e}", self.config.notification.discord_webhook_url)
            return pd.DataFrame()
    ...
    def get_latest_market_indices(self) -> Dict[str, Dict[str, Any]]:
        ...
        try:
            ...
            send_message(f"최신 시장 지수 조회 실패: {e}", self.config.notification.discord_webhook_url)
            return {} 